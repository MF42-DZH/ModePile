...
public enum SeasonPerk {
    ...
    SPRING_ACTIVE(1, 600, 600, 5, 10, 20, 50), // 2x levels from line clears, and 2x badge gain while active.
    SUMMER_PASSIVE(1), // +3 to visible next queue length. Guarantees an I-piece every 50 pieces (pushes back the next queue).
    SUMMER_ACTIVE(1, 250, 0, 2, 5, 10, 25), // Get two I-pieces instantly, pushing the next queue back.
    AUTUMN_PASSIVE(1), // Season badge benefits against gimmicks work 2x better.
    AUTUMN_ACTIVE(1, 800, 0, 5, 10, 20, 40), // Freefalls the field.
    WINTER_PASSIVE(1), // +6 frames of lock delay, always.
    WINTER_ACTIVE(1, 1000, 300, 5, 10, 20, 50); // Freezes gravity and gives increased lock delay (= 180) while active.
    ...
    private static class Descriptions {
        ...
        private Descriptions(float baseScale) {
            ...
            springPassive = GameTextUtilities.TextBlock.of(
                ...
                GameTextUtilities.Text.custom("(+1 LV/CLEAR, +0.1 BADGE/GAIN)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );

            springActive = GameTextUtilities.TextBlock.of(
                GameTextUtilities.TextJustification.LEFT,
                GameTextUtilities.Text.custom("\"UNBOUNDED NATURE\"", EventReceiver.COLOR_GREEN, baseScale), GameTextUtilities.Text.newLine(),
                GameTextUtilities.Text.blankLine(1f),
                ...
                GameTextUtilities.Text.custom("(2X LV/CLEAR, 2X BADGE/GAIN WHEN ACTIVE)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );
            ...
            summerPassive = GameTextUtilities.TextBlock.of(
                ...
                GameTextUtilities.Text.custom("(+3 NEXTS, GET EXTRA I-PIECE REGULARLY)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );

            summerActive = GameTextUtilities.TextBlock.of(
                GameTextUtilities.TextJustification.LEFT,
                GameTextUtilities.Text.custom("\"DROUGHT QUENCHER\"", EventReceiver.COLOR_YELLOW, baseScale), GameTextUtilities.Text.newLine(),
                GameTextUtilities.Text.blankLine(1f),
                ...
                GameTextUtilities.Text.custom("(GET 2 I-PIECES WHEN ACTIVATED)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );
            ...
            autumnPassive = GameTextUtilities.TextBlock.of(
                ...
                GameTextUtilities.Text.custom("(BADGE EFFECTS 2X UPON GIMMICKS)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );

            autumnActive = GameTextUtilities.TextBlock.of(
                GameTextUtilities.TextJustification.LEFT,
                GameTextUtilities.Text.custom("\"INTENSE GRAVITAS\"", EventReceiver.COLOR_ORANGE, baseScale), GameTextUtilities.Text.newLine(),
                GameTextUtilities.Text.blankLine(1f),
                ...
                GameTextUtilities.Text.custom("(FREEFALL STACK WHEN ACTIVATED)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );

            winterPassive = GameTextUtilities.TextBlock.of(
                GameTextUtilities.TextJustification.LEFT,
                GameTextUtilities.Text.custom("\"STOIC'S WILL\"", EventReceiver.COLOR_ORANGE, baseScale), GameTextUtilities.Text.newLine(),
                GameTextUtilities.Text.blankLine(1f),
                ...
                GameTextUtilities.Text.custom("(+6F LOCK DELAY)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );

            winterActive = GameTextUtilities.TextBlock.of(
                GameTextUtilities.TextJustification.LEFT,
                GameTextUtilities.Text.custom("\"CHILL OUT\"", EventReceiver.COLOR_ORANGE, baseScale), GameTextUtilities.Text.newLine(),
                GameTextUtilities.Text.blankLine(1f),
                ...
                GameTextUtilities.Text.custom("(0G, 180F LOCK DELAY WHILE ACTIVE)", EventReceiver.COLOR_WHITE, baseScale * 0.75f)
            );
        }
    }
