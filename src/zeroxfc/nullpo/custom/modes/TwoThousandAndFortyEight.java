...
public class TwoThousandAndFortyEight extends DummyMode {
        ...
        if ((engine.stat == GameEngine.STAT_SETTING) || ((engine.stat == GameEngine.STAT_RESULT) && (owner.replayMode == false))) {
            if (!owner.replayMode) {
                ...
                if (showPlayerStats) {
                    ...
                    if (!playerProperties.isLoggedIn())
                        receiver.drawScoreFont(engine, playerID, base, topY + MAX_RANKING + 2, "(NOT LOGGED IN)\n(E:LOG IN)");
                    if (playerProperties.isLoggedIn())
                        receiver.drawScoreFont(engine, playerID, base, topY + MAX_RANKING + 5, "F:SWITCH RANK SCREEN", EventReceiver.COLOR_GREEN);
            ...
            int sc;
            if (lastScoreTime <= 8) {
                sc = (int) Interpolation.tanStep(lastScore, engine.statistics.score, (double) lastScoreTime / 8.0);
            } else {
            ...
            if (mainGrid != null) {
                ...
                receiver.drawScoreFont(engine, playerID, base, 10, mainGrid.getMaxSquare() + "/2048");
    ...
    public void renderResult(GameEngine engine, int playerID) {
        ...
        receiver.drawMenuFont(engine, playerID, 0, 1, String.format("%8s", mainGrid.getMaxSquare()));

        receiver.drawMenuFont(engine, playerID, 0, 2, "SCORE", EventReceiver.COLOR_BLUE);
        receiver.drawMenuFont(engine, playerID, 0, 3, String.format("%8s", engine.statistics.score));
