...
public class ExpressShipping extends PuzzleGameEngine {
            ...
            if (mouseControl.getMouseClick(MouseParser.MouseButton.LEFT)) {
                ...
                if (monominoConveyorBelt != null) {
                    ...
                    if (checkPieceFit(engine, monominoConveyorBelt, fX, fY)) {
                        ...
                        if (getFieldLines(engine, playerID) - lines > 0)
                            engine.playSE("erase" + (getFieldLines(engine, playerID) - lines));
                        monominoConveyorBelt = null;
                        monominoesLeft--;
                    } else {
        ...
        if ((engine.stat == GameEngine.STAT_SETTING) || ((engine.stat == GameEngine.STAT_RESULT) && (!owner.replayMode))) {
            ...
            if (!owner.replayMode) {
                ...
                if (showPlayerStats) {
                    ...
                    GameTextUtilities.drawAlignedScoreText(receiver, engine, playerID, false, 0, topY + RANKING_MAX + 2, GameTextUtilities.Text.ofBig(playerProperties.getNameDisplay()));
            ...
            receiver.drawMenuFont(engine, playerID, 0, 22, String.valueOf((int) Interpolation.tanStep(lastScore, engine.statistics.score, (double) scGetTime / 60.0)));
    ...
    private void drawPiece(GameEngine engine, int playerID, GamePiece piece) {
        ...
        if (col != -1)
            receiver.drawDirectFont(engine, playerID, x + (piece.getCursorOffset()[0] * 16) - 8, y + (piece.getCursorOffset()[1] * 16), str, col);
        // endregion POWER ACTIVE
    }
    ...
    private void drawMousePiece(GameEngine engine, int playerID) {
        ...
        if (col != -1)
            receiver.drawDirectFont(engine, playerID, x + (offset[0] * 16) - 16, y + (offset[1] * 16) - 8, str, col);
        // endregion POWER ACTIVE
    }
