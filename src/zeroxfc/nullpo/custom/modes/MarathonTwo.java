...
public class MarathonTwo extends MarathonModeBase {
    ...
    private static final Logger log = Logger.getLogger(MarathonTwo.class);
    /**
    ...
    public void playerInit(GameEngine engine, int playerID) {
        ...
        SoundLoader.Sounds.Scanner.loadAllSounds();
    ...
    private String randomSound() {
        if (soundList.size() > 0 && effectRandomiser != null)
            return soundList.get(effectRandomiser.nextInt(soundList.size()));
        else return "";
    }
    ...
    public void renderLast(GameEngine engine, int playerID) {
        ...
        if (engine.stat != GameEngine.STAT_SETTING && engine.stat != GameEngine.STAT_RESULT && spookyValue >= 50) {
            if (titleCoefficient < FRAME_COLOUR_FLUCTUATION_CHANCE * Math.pow(spookyValue / 50d, 3.5))
                receiver.drawScoreFont(engine, playerID, 0, 0, currentTitle, EventReceiver.COLOR_RED);
            else
                receiver.drawScoreFont(engine, playerID, 0, 0, glitchTimer > 0 ? currentTitle : getName(), EventReceiver.COLOR_GREEN);

            if (tableGameClearLines[goaltype] == -1) {
                if (subtextCoefficient < FRAME_COLOUR_FLUCTUATION_CHANCE * Math.pow(spookyValue / 50d, 3.5))
                    receiver.drawScoreFont(engine, playerID, 0, 1, "(" + currentSubtext + " GAME)", EventReceiver.COLOR_RED);
                else receiver.drawScoreFont(engine, playerID, 0, 1, "(ENDLESS GAME)", EventReceiver.COLOR_GREEN);
            } else {
                if (subtextCoefficient < FRAME_COLOUR_FLUCTUATION_CHANCE * Math.pow(spookyValue / 50d, 3.5))
                    receiver.drawScoreFont(engine, playerID, 0, 1, "(" + currentSubtext + " GAME)", EventReceiver.COLOR_RED);
                else
                    receiver.drawScoreFont(engine, playerID, 0, 1, "(" + (glitchTimer > 0 ? currentSubtext : tableGameClearLines[goaltype]) + " LINES GAME)", EventReceiver.COLOR_GREEN);
            }
        } else {
        ...
        if ((engine.stat == GameEngine.STAT_SETTING) || ((engine.stat == GameEngine.STAT_RESULT) && (!owner.replayMode))) {
            ...
            if (glitchTimer > 0)
                GameTextUtilities.drawRandomRainbowScoreString(receiver, engine, playerID, 0, 3, GameTextUtilities.randomString(5, renderRandomiser), renderRandomiser, 1f);
            ...
            if (glitchTimer > 0) {
                ...
                if ((lastscore == 0) || (scgettime >= 120))
                    ...
                    GameTextUtilities.drawRainbowScoreString(receiver, engine, playerID, 0, 4, strScore, GameTextUtilities.RAINBOW_ORDER[rainbowPhase / 6], 1f);
            }

            if (glitchTimer > 0)
                GameTextUtilities.drawRandomRainbowScoreString(receiver, engine, playerID, 0, 6, GameTextUtilities.randomString(4, renderRandomiser), renderRandomiser, 1f);
