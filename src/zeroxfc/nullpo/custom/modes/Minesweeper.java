		...
		if(engine.owner.replayMode == false) {
			...
			if(change != 0) {
				...
				case 2:
					...
					if (minePercentage < 0.1f || ((int)(length * height * (minePercentage / 100f)) == 0)) minePercentage = 99.9f;
					break;
				case 3:
					timeLimit += 60 * change;
			...
			if (mainGrid != null) {
				...
				if (engine.stat == GameEngine.STAT_CUSTOM) {
					...
					if (length > MAX_DIM) {
						...
						if (offsetX != 0) receiver.drawDirectFont(engine, playerID, foffsetX - (2 * size), foffsetY + (int)(9.5 * size), "<", EventReceiver.COLOR_YELLOW);
						if (offsetX != length - MAX_DIM) receiver.drawDirectFont(engine, playerID, foffsetX + (21 * size), foffsetY + (int)(9.5 * size), ">", EventReceiver.COLOR_YELLOW);
					}
					
					if (height > MAX_DIM) {
						if (offsetY != 0) receiver.drawDirectFont(engine, playerID, foffsetX + (int)(9.5 * size), foffsetY - (2 * size), "k", EventReceiver.COLOR_YELLOW);
						if (offsetY != length - MAX_DIM) receiver.drawDirectFont(engine, playerID, foffsetX + (int)(9.5 * size), foffsetY + (21 * size), "n", EventReceiver.COLOR_YELLOW);
					}
				}
			}
