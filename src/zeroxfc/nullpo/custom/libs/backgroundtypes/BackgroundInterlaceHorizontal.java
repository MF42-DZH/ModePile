...
public class BackgroundInterlaceHorizontal extends AnimatedBackgroundHook {
	...
	public void update() {
		...
		if (pulseTimer >= pulseTimerMax) {
			...
			for (int i = 0; i < chunks.length; i++) {
				final boolean left = chunks[i].getAnchorType() == ImageChunk.ANCHOR_POINT_TR;
				final int newDir = left ? ImageChunk.ANCHOR_POINT_TL : ImageChunk.ANCHOR_POINT_TR;
				final int[] newAnchorLoc = new int[] { left ? 0 : SCREEN_WIDTH, i * rowHeight };

				chunks[i].setAnchorType(newDir);
				chunks[i].setAnchorLocation(newAnchorLoc);
			}
		}
		...
		for (int i = 0; i < chunks.length; i++) {
			...
			double s = Math.sin(Math.PI * ((double)pulseTimer / pulseTimerMax));
